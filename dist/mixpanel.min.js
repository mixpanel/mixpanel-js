...
$browser:d.info.ab(J,X.vendor,ya),$referrer:x.referrer,$referring_domain:d.info.Ad(x.referrer),$device:d.info.Rc(J)}),{$current_url:d.info.Y(),$browser_version:d.info.Gb(J,X.vendor,ya),$screen_height:Cb.height,$screen_width:Cb.width,mp_lib:"web",$lib_version:"2.75.0-rc1",$insert_id:Ca(),time:d.timestamp()/1E3},d.sa(a))},qf:function(){return d.extend(d.sa({$os:d.info.td(),$browser:d.info.ab(J,X.vendor,ya)}),{$browser_version:d.info.Gb(J,X.vendor,ya)})},hf:function(){return d.sa({current_page_title:x.title,
...
b=this.Uc("$device_id"),c=Vb();K.log("Fetching flags for distinct ID: "+a);a=d.extend({distinct_id:a,device_id:b},this.o("context"));b=new URLSearchParams;b.set("context",JSON.stringify(a));b.set("token",this.Da("token"));b.set("mp_lib","web");b.set("$lib_version","2.75.0-rc1");a=this.Pb()+"?"+b.toString();this.wb=Date.now();return this.Ob=this.fetch.call(q,a,{method:"GET",headers:{Authorization:"Basic "+btoa(this.Da("token")+":"),traceparent:c}}).then(function(e){this.cc();return e.json().then(function(f){f=
