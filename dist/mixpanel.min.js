...
$referring_domain:d.info.Dc(A.referrer),$device:d.info.cc(H)}),{$current_url:d.info.Da(),$browser_version:d.info.cb(H,W.vendor,ta),$screen_height:sb.height,$screen_width:sb.width,mp_lib:"web",$lib_version:"2.64.0-rc1",$insert_id:za(),time:d.timestamp()/1E3},d.ba(a))},je:function(){return d.extend(d.ba({$os:d.info.wc(),$browser:d.info.Aa(H,W.vendor,ta)}),{$browser_version:d.info.cb(H,W.vendor,ta)})},fe:function(){return d.ba({current_page_title:A.title,current_domain:r.location.hostname,current_url_path:r.location.pathname,
...
f.length===e,k=[],h={};d.g(f,function(m){var t=m.payload;this.za&&!m.he&&(t=this.za(t));if(t){t.event&&t.properties&&(t.properties=d.extend({},t.properties,{mp_sent_by_lib_version:"2.64.0-rc1"}));var w=!0,u=m.id;u?5<(this.V[u]||0)&&(this.m("[dupe] item ID sent too many times, not sending",{item:m,T:f.length,Ke:this.V[u]}),w=!1):this.m("[dupe] found item with no ID",{item:m});w&&k.push(t)}h[m.id]=t},this);if(1>k.length)return this.ub=!1,this.Z(),D.resolve();var p=d.bind(function(){return this.ra.ke(d.map(f,
