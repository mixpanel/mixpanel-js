...
$browser:d.info.Ya(J,X.vendor,ya),$referrer:x.referrer,$referring_domain:d.info.Ad(x.referrer),$device:d.info.Rc(J)}),{$current_url:d.info.Y(),$browser_version:d.info.Eb(J,X.vendor,ya),$screen_height:Bb.height,$screen_width:Bb.width,mp_lib:"web",$lib_version:"2.73.0-rc3",$insert_id:Ca(),time:d.timestamp()/1E3},d.sa(a))},pf:function(){return d.extend(d.sa({$os:d.info.td(),$browser:d.info.Ya(J,X.vendor,ya)}),{$browser_version:d.info.Eb(J,X.vendor,ya)})},gf:function(){return d.sa({current_page_title:x.title,
...
b=this.Uc("$device_id"),c=Tb();K.log("Fetching flags for distinct ID: "+a);a=d.extend({distinct_id:a,device_id:b},this.o("context"));b=new URLSearchParams;b.set("context",JSON.stringify(a));b.set("token",this.Da("token"));b.set("mp_lib","web");b.set("$lib_version","2.73.0-rc3");a=this.Nb()+"?"+b.toString();this.ub=Date.now();return this.Mb=this.fetch.call(n,a,{method:"GET",headers:{Authorization:"Basic "+btoa(this.Da("token")+":"),traceparent:c}}).then(function(e){this.ac();return e.json().then(function(f){f=
