...
$browser:c.info.ga(A,I.vendor,Y),$referrer:u.referrer,$referring_domain:c.info.Hb(u.referrer),$device:c.info.rb(A)}),{$current_url:x.location.href,$browser_version:c.info.Fa(A,I.vendor,Y),$screen_height:Z.height,$screen_width:Z.width,mp_lib:"web",$lib_version:"2.47.0-rc2",$insert_id:ea(),time:c.timestamp()/1E3})},Jc:function(){return c.extend(c.ra({$os:c.info.Cb(),$browser:c.info.ga(A,I.vendor,Y)}),{$browser_version:c.info.Fa(A,I.vendor,Y)})},Fc:function(){return c.ra({current_page_title:u.title,
...
d=(new Date).getTime(),f=this.C,h=this.W.tc(f),e=[],s={};c.a(h,function(a){var b=a.payload;this.fa&&!a.Hc&&(b=this.fa(b));if(b){b.event&&b.properties&&(b.properties=c.extend({},b.properties,{mp_sent_by_lib_version:"2.47.0-rc2"}));var d=m,f=a.id;if(f){if(5<(this.F[f]||0))this.h("[dupe] item ID sent too many times, not sending",{item:a,C:h.length,dd:this.F[f]}),d=D}else this.h("[dupe] found item with no ID",{item:a});d&&e.push(b)}s[a.id]=b},this);if(1>e.length)this.Y();else{this.Kb=m;var i=c.bind(function(e){this.Kb=
