...
$browser:d.info.Va(J,X.vendor,xa),$referrer:x.referrer,$referring_domain:d.info.yd(x.referrer),$device:d.info.Pc(J)}),{$current_url:d.info.W(),$browser_version:d.info.Bb(J,X.vendor,xa),$screen_height:Bb.height,$screen_width:Bb.width,mp_lib:"web",$lib_version:"2.72.0-rc2",$insert_id:Ca(),time:d.timestamp()/1E3},d.qa(a))},mf:function(){return d.extend(d.qa({$os:d.info.rd(),$browser:d.info.Va(J,X.vendor,xa)}),{$browser_version:d.info.Bb(J,X.vendor,xa)})},ef:function(){return d.qa({current_page_title:x.title,
...
b=this.Sc("$device_id"),c=Ub();K.log("Fetching flags for distinct ID: "+a);a=d.extend({distinct_id:a,device_id:b},this.o("context"));b=new URLSearchParams;b.set("context",JSON.stringify(a));b.set("token",this.Ba("token"));b.set("mp_lib","web");b.set("$lib_version","2.72.0-rc2");a=this.Kb()+"?"+b.toString();this.rb=Date.now();return this.Jb=this.fetch.call(q,a,{method:"GET",headers:{Authorization:"Basic "+btoa(this.Ba("token")+":"),traceparent:c}}).then(function(e){this.Zb();return e.json().then(function(f){f=
