...
$browser:c.info.ga(A,I.vendor,Y),$referrer:u.referrer,$referring_domain:c.info.Hb(u.referrer),$device:c.info.rb(A)}),{$current_url:x.location.href,$browser_version:c.info.Fa(A,I.vendor,Y),$screen_height:Z.height,$screen_width:Z.width,mp_lib:"web",$lib_version:"2.47.0-rc1",$insert_id:ea(),time:c.timestamp()/1E3})},Jc:function(){return c.extend(c.ra({$os:c.info.Cb(),$browser:c.info.ga(A,I.vendor,Y)}),{$browser_version:c.info.Fa(A,I.vendor,Y)})},Fc:function(){return c.ra({current_page_title:u.title,
current_domain:x.location.hostname,current_url_path:x.location.pathname,current_url_protocol:x.location.protocol,current_url_search:x.location.search})}};var Ga=/[a-z0-9][a-z0-9-]*\.[a-z]+$/i,Fa=/[a-z0-9][a-z0-9-]+\.[a-z.]{2,6}$/i,$=q,aa=q;if("undefined"!==typeof JSON)$=JSON.stringify,aa=JSON.parse;$=$||c.ba;aa=aa||c.O;c.toArray=c.L;c.isObject=c.e;c.JSONEncode=c.ba;c.JSONDecode=c.O;c.isBlockedUA=c.xb;c.isEmptyObject=c.na;c.info=c.info;c.info.device=c.info.rb;c.info.browser=c.info.ga;c.info.browserVersion=
c.info.Fa;c.info.properties=c.info.V;E.prototype.ja=function(){};E.prototype.Ka=function(){};E.prototype.Da=function(){};E.prototype.Oa=function(a){this.zb=a;return this};E.prototype.o=function(a,b,d,f){var h=this,g=c.rc(a);if(0===g.length)p.error("The DOM query ("+a+") returned 0 elements");else return c.a(g,function(a){c.Ib(a,this.Db,function(a){var c={},g=h.ja(d,this),e=h.zb.c("track_links_timeout");h.Ka(a,this,c);window.setTimeout(h.Xb(f,g,c,m),e);h.zb.o(b,g,h.Xb(f,g,c))})},this),m};E.prototype.Xb=
function(a,b,c,f){var f=f||D,h=this;return function(){if(!c.oc)c.oc=m,a&&a(f,b)===D||h.Da(b,c,f)}};E.prototype.ja=function(a,b){return"function"===typeof a?a(b):c.extend({},a)};c.wb(L,E);L.prototype.ja=function(a,b){var c=L.bd.ja.apply(this,arguments);if(b.href)c.url=b.href;return c};L.prototype.Ka=function(a,b,c){c.Ab=2===a.which||a.metaKey||a.ctrlKey||"_blank"===b.target;c.href=b.href;c.Ab||a.preventDefault()};L.prototype.Da=function(a,b){b.Ab||setTimeout(function(){window.location=b.href},0)};
c.wb(T,E);T.prototype.Ka=function(a,b,c){c.element=b;a.preventDefault()};T.prototype.Da=function(a,b){setTimeout(function(){b.element.submit()},0)};var Qa=ga("lock");qa.prototype.Ya=function(a,b,c){function f(){j.setItem(u,"1");try{a()}finally{j.removeItem(u),j.getItem(p)===k&&j.removeItem(p),j.getItem(n)===k&&j.removeItem(n)}}function h(){j.setItem(n,k);e(g,function(){j.getItem(n)===k?f():i(function(){j.getItem(p)!==k?h():e(function(){return!j.getItem(u)},f)})})}function g(){var a=j.getItem(p);if(a&&
a!==k)return D;j.setItem(p,k);if(j.getItem(p)===k)return m;if(!U(j,m))throw Error("localStorage support dropped while acquiring lock");return D}function e(a,b){a()?b():i(function(){e(a,b)})}function i(a){(new Date).getTime()-r>v?(Qa.error("Timeout waiting for mutex on "+o+"; clearing lock. ["+k+"]"),j.removeItem(u),j.removeItem(p),h()):setTimeout(function(){try{a()}catch(c){b&&b(c)}},w*(Math.random()+0.1))}!c&&"function"!==typeof b&&(c=b,b=q);var k=c||(new Date).getTime()+"|"+Math.random(),r=(new Date).getTime(),
o=this.K,w=this.Gb,v=this.Vb,j=this.i,n=o+":X",p=o+":Y",u=o+":Z";try{if(U(j,m))h();else throw Error("localStorage support check failed");}catch(t){b&&b(t)}};var pa=ga("batch");G.prototype.Ja=function(a,b,d){var f={id:ea(),flushAfter:(new Date).getTime()+2*b,payload:a};this.Ra.Ya(c.bind(function(){var b;try{var c=this.X();c.push(f);(b=this.Ua(c))&&this.G.push(f)}catch(e){this.h("Error enqueueing item",a),b=D}d&&d(b)},this),c.bind(function(a){this.h("Error acquiring storage lock",a);d&&d(D)},this),
this.pa)};G.prototype.tc=function(a){var b=this.G.slice(0,a);if(b.length<a){var d=this.X();if(d.length){var f={};c.a(b,function(a){f[a.id]=m});for(var h=0;h<d.length;h++){var g=d[h];if((new Date).getTime()>g.flushAfter&&!f[g.id]&&(g.Hc=m,b.push(g),b.length>=a))break}}}return b};G.prototype.Kc=function(a,b){var d={};c.a(a,function(a){d[a]=m});this.G=oa(this.G,d);var f=c.bind(function(){var b;try{var c=this.X(),c=oa(c,d);if(b=this.Ua(c))for(var c=this.X(),f=0;f<c.length;f++){var e=c[f];if(e.id&&d[e.id])return this.h("Item not removed from storage"),
...
d=(new Date).getTime(),f=this.C,h=this.W.tc(f),e=[],s={};c.a(h,function(a){var b=a.payload;this.fa&&!a.Hc&&(b=this.fa(b));if(b){b.event&&b.properties&&(b.properties=c.extend({},b.properties,{mp_sent_by_lib_version:"2.47.0-rc1"}));var d=m,f=a.id;if(f){if(5<(this.F[f]||0))this.h("[dupe] item ID sent too many times, not sending",{item:a,C:h.length,dd:this.F[f]}),d=D}else this.h("[dupe] found item with no ID",{item:a});d&&e.push(b)}s[a.id]=b},this);if(1>e.length)this.Y();else{this.Kb=m;var i=c.bind(function(e){this.Kb=
