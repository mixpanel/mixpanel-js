...
ga:function(a){"object"!==typeof a&&(a={});return c.extend(c.la({$os:c.info.$b(),$browser:c.info.ta(A,K.vendor,da),$referrer:t.referrer,$referring_domain:c.info.fc(t.referrer),$device:c.info.Nb(A)}),{$current_url:c.info.Ta(),$browser_version:c.info.Qa(A,K.vendor,da),$screen_height:ea.height,$screen_width:ea.width,mp_lib:"web",$lib_version:"2.57.1",$insert_id:ka(),time:c.timestamp()/1E3},c.la(a))},kd:function(){return c.extend(c.la({$os:c.info.$b(),$browser:c.info.ta(A,K.vendor,da)}),{$browser_version:c.info.Qa(A,
...
g.length===f,v=[],i={};c.a(g,function(a){var b=a.payload;this.sa&&!a.jd&&(b=this.sa(b));if(b){b.event&&b.properties&&(b.properties=c.extend({},b.properties,{mp_sent_by_lib_version:"2.57.1"}));var d=l,f=a.id;if(f){if(5<(this.M[f]||0))this.h("[dupe] item ID sent too many times, not sending",{item:a,I:g.length,Id:this.M[f]}),d=B}else this.h("[dupe] found item with no ID",{item:a});d&&v.push(b)}i[a.id]=b},this);if(1>v.length)return this.jb=B,this.R(),y.resolve();var j=c.bind(function(){return this.ha.ld(c.map(g,
