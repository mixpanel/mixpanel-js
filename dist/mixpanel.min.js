...
$browser:c.info.ja(A,I.vendor,Y),$referrer:u.referrer,$referring_domain:c.info.Ob(u.referrer),$device:c.info.xb(A)}),{$current_url:w.location.href,$browser_version:c.info.Ha(A,I.vendor,Y),$screen_height:Z.height,$screen_width:Z.width,mp_lib:"web",$lib_version:"2.48.0-rc2",$insert_id:ea(),time:c.timestamp()/1E3})},Nc:function(){return c.extend(c.ua({$os:c.info.Jb(),$browser:c.info.ja(A,I.vendor,Y)}),{$browser_version:c.info.Ha(A,I.vendor,Y)})},Kc:function(){return c.ua({current_page_title:u.title,
current_domain:w.location.hostname,current_url_path:w.location.pathname,current_url_protocol:w.location.protocol,current_url_search:w.location.search})}};var Ga=/[a-z0-9][a-z0-9-]*\.[a-z]+$/i,Fa=/[a-z0-9][a-z0-9-]+\.[a-z.]{2,6}$/i,$=q,aa=q;if("undefined"!==typeof JSON)$=JSON.stringify,aa=JSON.parse;$=$||c.ea;aa=aa||c.P;c.toArray=c.M;c.isObject=c.e;c.JSONEncode=c.ea;c.JSONDecode=c.P;c.isBlockedUA=c.Eb;c.isEmptyObject=c.pa;c.info=c.info;c.info.device=c.info.xb;c.info.browser=c.info.ja;c.info.browserVersion=
c.info.Ha;c.info.properties=c.info.X;E.prototype.ma=function(){};E.prototype.Ma=function(){};E.prototype.Fa=function(){};E.prototype.Ra=function(a){this.Gb=a;return this};E.prototype.o=function(a,b,d,f){var h=this,g=c.xc(a);if(0===g.length)o.error("The DOM query ("+a+") returned 0 elements");else return c.a(g,function(a){c.Pb(a,this.Kb,function(a){var c={},g=h.ma(d,this),e=h.Gb.c("track_links_timeout");h.Ma(a,this,c);window.setTimeout(h.bc(f,g,c,m),e);h.Gb.o(b,g,h.bc(f,g,c))})},this),m};E.prototype.bc=
function(a,b,c,f){var f=f||D,h=this;return function(){if(!c.uc)c.uc=m,a&&a(f,b)===D||h.Fa(b,c,f)}};E.prototype.ma=function(a,b){return"function"===typeof a?a(b):c.extend({},a)};c.Db(L,E);L.prototype.ma=function(a,b){var c=L.fd.ma.apply(this,arguments);if(b.href)c.url=b.href;return c};L.prototype.Ma=function(a,b,c){c.Hb=2===a.which||a.metaKey||a.ctrlKey||"_blank"===b.target;c.href=b.href;c.Hb||a.preventDefault()};L.prototype.Fa=function(a,b){b.Hb||setTimeout(function(){window.location=b.href},0)};
c.Db(T,E);T.prototype.Ma=function(a,b,c){c.element=b;a.preventDefault()};T.prototype.Fa=function(a,b){setTimeout(function(){b.element.submit()},0)};var Qa=ga("lock");qa.prototype.cb=function(a,b,c){function f(){j.setItem(u,"1");try{a()}finally{j.removeItem(u),j.getItem(o)===k&&j.removeItem(o),j.getItem(n)===k&&j.removeItem(n)}}function h(){j.setItem(n,k);e(g,function(){j.getItem(n)===k?f():i(function(){j.getItem(o)!==k?h():e(function(){return!j.getItem(u)},f)})})}function g(){var a=j.getItem(o);if(a&&
a!==k)return D;j.setItem(o,k);if(j.getItem(o)===k)return m;if(!U(j,m))throw Error("localStorage support dropped while acquiring lock");return D}function e(a,b){a()?b():i(function(){e(a,b)})}function i(a){(new Date).getTime()-p>v?(Qa.error("Timeout waiting for mutex on "+s+"; clearing lock. ["+k+"]"),j.removeItem(u),j.removeItem(o),h()):setTimeout(function(){try{a()}catch(c){b&&b(c)}},x*(Math.random()+0.1))}!c&&"function"!==typeof b&&(c=b,b=q);var k=c||(new Date).getTime()+"|"+Math.random(),p=(new Date).getTime(),
s=this.L,x=this.Nb,v=this.$b,j=this.i,n=s+":X",o=s+":Y",u=s+":Z";try{if(U(j,m))h();else throw Error("localStorage support check failed");}catch(t){b&&b(t)}};var pa=ga("batch");G.prototype.La=function(a,b,d){var f={id:ea(),flushAfter:(new Date).getTime()+2*b,payload:a};this.Ua.cb(c.bind(function(){var b;try{var c=this.$();c.push(f);(b=this.Xa(c))&&this.G.push(f)}catch(e){this.h("Error enqueueing item",a),b=D}d&&d(b)},this),c.bind(function(a){this.h("Error acquiring storage lock",a);d&&d(D)},this),
this.sa)};G.prototype.zc=function(a){var b=this.G.slice(0,a);if(b.length<a){var d=this.$();if(d.length){var f={};c.a(b,function(a){f[a.id]=m});for(var h=0;h<d.length;h++){var g=d[h];if((new Date).getTime()>g.flushAfter&&!f[g.id]&&(g.Mc=m,b.push(g),b.length>=a))break}}}return b};G.prototype.Oc=function(a,b){var d={};c.a(a,function(a){d[a]=m});this.G=oa(this.G,d);var f=c.bind(function(){var b;try{var c=this.$(),c=oa(c,d);if(b=this.Xa(c))for(var c=this.$(),f=0;f<c.length;f++){var e=c[f];if(e.id&&d[e.id])return this.h("Item not removed from storage"),
...
d=(new Date).getTime(),f=this.C,h=this.Y.zc(f),e=[],r={};c.a(h,function(a){var b=a.payload;this.ia&&!a.Mc&&(b=this.ia(b));if(b){b.event&&b.properties&&(b.properties=c.extend({},b.properties,{mp_sent_by_lib_version:"2.48.0-rc2"}));var d=m,f=a.id;if(f){if(5<(this.F[f]||0))this.h("[dupe] item ID sent too many times, not sending",{item:a,C:h.length,hd:this.F[f]}),d=D}else this.h("[dupe] found item with no ID",{item:a});d&&e.push(b)}r[a.id]=b},this);if(1>e.length)this.aa();else{this.Rb=m;var i=c.bind(function(e){this.Rb=
